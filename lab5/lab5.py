...
class NeuralNetwork:
    ...
    def forward(self):
        ...
        self.output = softmax(Z2)
        # self.output = Z2
        loss = np.sum(((self.output - self.Y)**2))
        # print(Z2.shape)
        # loss = self.calc_loss(self.output)
        # print(loss)
        return self.output.T, loss
    ...
    def calc_loss(self, output):
        loss = (1./self.sample_count) * (-np.dot(self.Y,np.log(output).T) - np.dot(1-self.Y, np.log(1-output).T))    
        return loss
